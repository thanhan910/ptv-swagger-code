  ...
  class V3Outlet
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::V3Outlet` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::V3Outlet`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [outlet_slid_spid, outlet_name, outlet_business, outlet_latitude, outlet_longitude, outlet_suburb, outlet_postcode, outlet_business_hour_mon, outlet_business_hour_tue, outlet_business_hour_wed, outlet_business_hour_thur, outlet_business_hour_fri, outlet_business_hour_sat, outlet_business_hour_sun, outlet_notes].hash
    end
