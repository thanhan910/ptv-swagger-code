...
open class PatternsAPI {
    ...
    /**
     ...
     - parameter runRef: (path) The run_ref is the identifier of a run as returned by the departures/_* and runs/_* endpoints. WARNING, run_id is deprecated. Use run_ref instead.      - parameter routeType: (path) Number identifying transport mode; values returned via RouteTypes API      - parameter expand: (query) List of objects to be returned in full (i.e. expanded) - options include: All, Stop, Route, Run, Direction, Disruption, VehiclePosition, VehicleDescriptor and None. Default is Disruption. Run must be expanded to receive VehiclePosition and VehicleDescriptor information.      - parameter stopId: (query) Filter by stop_id; values returned by Stops API (optional)     - parameter dateUtc: (query) Filter by the date and time of the request (ISO 8601 UTC format) (optional)     - parameter includeSkippedStops: (query) Include any skipped stops in a stopping pattern. Defaults to false. (optional)     - parameter includeGeopath: (query) Indicates if geopath data will be returned (default &#x3D; false) (optional)     - parameter token: (query) Please ignore (optional)     - parameter devid: (query) Your developer id (optional)     - parameter signature: (query) Authentication signature for request (optional)
